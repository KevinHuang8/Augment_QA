...
class Generator(object):
    ...
    def __init__(self, args, api_key_file: str='key.txt',
        ...
        self.args = args
        # We don't need real API keys with local vLLM
        self.keys = ["dummy"]
    ...
    def build_generate_prompt(self, data_item: Dict, generate_type: Tuple,
                              table_only: bool = False, report_ahead: bool = False,
                              datasetname: str = None, info_title: str = None,
                              max_row: int = None):
        """Build the generate prompt."""
        return self.prompt_builder.build_generate_prompt(
            **data_item, generate_type=generate_type, table_only=table_only,
            report_ahead=report_ahead, datasetname=datasetname,
            info_title=info_title, max_row=max_row
        )

    def generate_one_pass(self, prompts: List[Tuple], verbose: bool = False,
    ...
    def _call_llm_api(self, prompt: Union[str, List], max_tokens, temperature: float,
                      top_p: float, n: int, stop: List[str],
